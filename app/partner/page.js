...
export default function PartnerDashboard() {
  ...
  useEffect(() => {
    if (!mounted || (!isAdmin && (!entityType || !selectedEntityName))) return;

    const leadsChannel = supabase
      .channel(`partner-leads-${entityType || 'admin'}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'leads' },
        () => fetchIncomingLeads(entityType, selectedEntityName, isAdmin)
      )
      .subscribe();
  ...
  }, [mounted, entityType, selectedEntityName, isAdmin]);

  const normalizeTaxProfile = (record = {}) => ({
    vatRegistered: Boolean(record.vat_registered ?? record.is_vat_registered ?? false),
    vatRate: Math.max(Number(record.vat_rate ?? 0) || 0, 0),
    pricesIncludeTax: Boolean(record.prices_include_tax ?? false)
  });

  const normalizeEntityName = (value = '') =>
    String(value || '').replace(/[\[\]"]/g, '').toLowerCase().replace(/[\s_-]/g, '').trim();
  ...
  const calculateTaxBreakdown = (amount, profile = taxProfile) => {
    const safeAmount = Math.max(Number(amount) || 0, 0);
    const rate = profile.vatRegistered ? Math.max(Number(profile.vatRate) || 0, 0) : 0;

    if (rate === 0) return { base: safeAmount, vat: 0, total: safeAmount };

    if (profile.pricesIncludeTax) {
      const base = safeAmount / (1 + rate / 100);
      return { base, vat: safeAmount - base, total: safeAmount };
    }
  ...
  const generateAndDownloadOfficialPDF = async (lead = {}, proposalData = {}, type = 'provider') => {
    ...
    try {
      ...
      const proposalTaxProfile = {
        vatRegistered: Boolean(proposalData?.vat_registered),
        vatRate: Number(proposalData?.vat_rate) || 0,
        pricesIncludeTax: Boolean(proposalData?.prices_include_tax)
      };
      const proposalAmount = type === 'provider' ? basePrice : laborCost;
      const taxBreakdown = calculateTaxBreakdown(proposalAmount, proposalTaxProfile);
        ...
        <div style="padding: 40px; font-family: Arial, sans-serif; color: #1d1d1f; max-width: 700px; margin: auto; background: #ffffff; border: 1px solid #e2e8f0;">
          ...
          <div style="background: #f8fafc; padding: 20px; border-radius: 10px; margin-bottom: 20px; font-size: 12px; line-height: 1.8; border: 1px solid #e2e8f0;">
            ...
            ${type === 'provider' ? `
              ...
              <p style="margin: 0;"><strong>BASE PRICE:</strong>  ₱ ${taxBreakdown.base.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
              <p style="margin: 0;"><strong>RECOMMENDED PRICE:</strong>  ₱ ${recommendedPrice.toLocaleString()}</p>
            ` : `
              ...
              <p style="margin: 0;"><strong>LABOR COST:</strong>  ₱ ${taxBreakdown.base.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
            `}
            <p style="margin: 8px 0 0;"><strong>TAX PROFILE:</strong> ${proposalTaxProfile.vatRegistered ? `VAT ${proposalTaxProfile.vatRate}%` : 'NON-VAT'}</p>
            <p style="margin: 0;"><strong>VAT AMOUNT:</strong>  ₱ ${taxBreakdown.vat.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
            <p style="margin: 0;"><strong>PARTNER PAYABLE TOTAL:</strong>  ₱ ${taxBreakdown.total.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
          </div>
  ...
  async function saveDetailedQuotation(leadId, currentEntityType, isFinalQuote = false) {
    ...
    let proposalPayload = currentEntityType === 'provider' ? {
      ...
      vat_registered: taxProfile.vatRegistered,
      vat_rate: taxProfile.vatRate,
      prices_include_tax: taxProfile.pricesIncludeTax
    } : {
      lead_id: leadId,
      entity_name: cleanEntityName,
    ...
    const partnerBaseAmount = currentEntityType === 'provider'
      ? Number(inputState.basePrice || 0)
      : Number(inputState.laborCost || 0);
    const dpPercentVal = inputState.downpaymentPercent !== undefined ? Number(inputState.downpaymentPercent) : 30;

    // FINAL QUOTE: the selected provider/installer must set a fresh final price here.
    // Do not reuse the earlier proposal price as the final contract price.
    if (!Number.isFinite(partnerBaseAmount) || partnerBaseAmount <= 0) {
      alert('Enter the final project price before locking the quote.');
      return;
    }
    const partnerTaxBreakdown = calculateTaxBreakdown(partnerBaseAmount);
    const calculatedDownpayment = (partnerTaxBreakdown.total * dpPercentVal) / 100;
    const calculatedBalance = partnerTaxBreakdown.total - calculatedDownpayment;
    ...
    let leadUpdatePayload = currentEntityType === 'provider' ? {
      ...
      provider_vat_registered: taxProfile.vatRegistered,
      provider_vat_rate: taxProfile.vatRate,
      provider_prices_include_tax: taxProfile.pricesIncludeTax
    } : {
      installer_downpayment: calculatedDownpayment,
      installer_downpayment_pct: dpPercentVal,
      installer_balance: calculatedBalance,
      installer_vat_registered: taxProfile.vatRegistered,
      installer_vat_rate: taxProfile.vatRate,
      installer_prices_include_tax: taxProfile.pricesIncludeTax
    };

    try {
      const notificationResponse = await fetch('/api/v1/solar-leads', {
        method: 'PATCH',
        ...
        body: JSON.stringify({
          ...
          ...proposalPayload,
          ...(isFinalQuote ? leadUpdatePayload : {})
        })
      });
      const notificationResult = await notificationResponse.json();
      if (!notificationResponse.ok || !notificationResult.success) {
        throw new Error(notificationResult.error || 'Unable to finalize the quotation notification.');
      }

      if (isFinalQuote) {
        const percentageColumn = currentEntityType === 'provider'
          ? 'provider_downpayment_pct'
          : 'installer_downpayment_pct';
        const { error: percentageError } = await supabase
          .from('leads')
          .update({ [percentageColumn]: dpPercentVal })
          .eq('id', leadId);
        if (percentageError) throw percentageError;

        // Persist the FINAL-quote lock independently for provider and installer.
        // Old proposal values do not count as a final quote and one role cannot lock the other.
        ...
        const existingQuoteDetails = latestLead?.quote_details && typeof latestLead.quote_details === 'object'
          ? latestLead.quote_details
          : {};
        const finalQuoteLockKey = currentEntityType === 'provider'
          ? 'provider_final_quote_locked'
          : 'installer_final_quote_locked';
        const { error: lockError } = await supabase
          .from('leads')
          .update({
            quote_details: {
              ...existingQuoteDetails,
              [finalQuoteLockKey]: true
            }
          })
          .eq('id', leadId);
      ...
      alert(isFinalQuote
        ? `${currentEntityType.toUpperCase()} final quote and payment pricing successfully saved & locked!`
        : `${currentEntityType.toUpperCase()} proposal successfully saved!`);
  ...
  async function checkActiveSession() {
    try {
      ...
      if (providerData) {
        ...
        setTaxProfile(normalizeTaxProfile(providerData));
        fetchPartnerCargo(formattedProvider);
        fetchIncomingLeads('provider', formattedProvider, false);
        setCheckingAuth(false);
        return;
      }
      ...
      if (installerData) {
        ...
        setTaxProfile(normalizeTaxProfile(installerData));
        fetchInstallerCargo(formattedInstaller);
        fetchIncomingLeads('installer', formattedInstaller, false);
        setCheckingAuth(false);
        return;
      }
        ...
        <body>
          <div class="container">
            ...
            <div class="section">
              ...
              <span class="value"> ₱ ${monthlyBill}</span>
            </div>
  ...
  async function handleAcceptLead(leadId) {
    ...
    try {
      ...
      const response = await fetch('/api/v1/solar-leads', {
        ...
        body: JSON.stringify({
          lead_id: leadId,
          selection_type: entityType,
          entity_name: cleanName
        })
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || 'Unable to accept this lead.');
      await fetchIncomingLeads();
  ...
  async function fetchIncomingLeads(roleOverride = entityType, entityNameOverride = selectedEntityName, adminOverride = isAdmin) {
    try {
      const activeEntityType = roleOverride;
      const activeIsAdmin = adminOverride;
      const cleanEntityName = (entityNameOverride || '').toLowerCase().replace(/[\s_-]/g, '').trim();
      ...
      const filteredLeads = leadsData
        .filter(lead => {
          ...
          if (activeEntityType === 'provider') {
            if (assignedProv && assignedProv !== cleanEntityName) {
              return false;
            }
          } else if (activeEntityType === 'installer') {
      ...
      console.warn("Unable to refresh the incoming leads pool:", err?.message || err);
      setIncomingLeads([]);
    }
  }
  ...
  async function handleRejectLead(leadId) {
    ...
    try {
      ...
      if (entityType === 'provider') {
        let currentRejected = parseEntityList(leadObj?.partner_rejected_by);

        if (!currentRejected.some(name => normalizeEntityName(name) === normalizeEntityName(cleanName))) {
        ...
        let currentRejected = parseEntityList(leadObj?.installer_rejected_by);
  ...
  async function handleProviderPriceChange(id, currentPrice) {
    const newPriceInput = prompt(`CURRENT PRICE:  ₱ ${currentPrice}\nEnter new price:`, currentPrice);
 ...
 const cleanCurrentEntity = (selectedEntityName || '').toLowerCase().replace(/[\s_-]/g, '').trim();
  ...
  const newIncomingLeadsCount = incomingLeads.filter(lead => {
    ...
    if ((assignedEntity || '').toLowerCase().replace(/[\s_-]/g, '').trim() === cleanCurrentEntity) {
      return false;
    }

    const rejectedList = parseEntityList(entityType === 'provider' ? lead.partner_rejected_by : lead.installer_rejected_by);
    const isRejectedByMe = rejectedList.some(item => normalizeEntityName(item) === cleanCurrentEntity);
    if (isRejectedByMe) return false;

    const myProposal = entityType === 'provider'
      ? (lead.provider_proposals || []).find(p => (p.entity_name || '').toLowerCase().replace(/[\s_-]/g, '').trim() === cleanCurrentEntity)
      : (lead.installer_proposals || []).find(p => (p.entity_name || '').toLowerCase().replace(/[\s_-]/g, '').trim() === cleanCurrentEntity);
    ...
    const acceptedList = parseEntityList(entityType === 'provider' ? lead.partner_accepted_by : lead.installer_accepted_by);
    const isAcceptedByMe = acceptedList.some(item => normalizeEntityName(item) === cleanCurrentEntity);
 ...
 const pendingAcceptedLeadsList = incomingLeads.filter(lead => {
    ...
    const myProposal = entityType === 'provider'
      ? (lead.provider_proposals || []).find(p => normalizeEntityName(p.entity_name) === cleanCurrentEntity)
      : (lead.installer_proposals || []).find(p => normalizeEntityName(p.entity_name) === cleanCurrentEntity);
...
return (
    <div className="partner-canvas">
      <style jsx global>{`
        ...
        .flicker-dot { width: 7px; height: 7px; border-radius: 999px; display: inline-block; animation: lead-alert-pulse 1.2s ease-in-out infinite; }
                    ...
                    <div style={{minWidth: 0}}>
                      ...
                      <div style={{display: 'flex', gap: '10px', marginTop: '6px', fontSize: '10px', fontFamily: 'monospace', flexWrap: 'wrap'}}>
                        <span style={{display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(255,255,255,0.15)', padding: '3px 8px', borderRadius: '6px'}}>
                          ...
                          <strong style={{color: '#86efac'}}>{newIncomingLeadsCount}</strong> incoming {newIncomingLeadsCount === 1 ? 'lead' : 'leads'}
                        </span>
                        ...
                        <span style={{display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(255,255,255,0.15)', padding: '3px 8px', borderRadius: '6px'}}>
                          ...
                          <strong style={{color: '#fde047'}}>{pendingAcceptedLeadsList.filter(lead => {
                            ...
                            return true;
                            }).length}</strong> pending {pendingAcceptedLeadsList.length === 1 ? 'lead' : 'leads'}
                        </span>
                      </div>
                    </div>
                  </div>
    ...
    {incomingLeads.filter(lead => {
      ...
      <div style={{background: '#fff', border: '1px dashed #bae6fd', padding: '20px 15px', borderRadius: '12px', textAlign: 'center', boxSizing: 'border-box'}}>
        <p style={{fontSize: '11px', color: '#64748b', fontFamily: 'monospace', margin: 0, lineHeight: '1.5'}}>
          No officially assigned clients yet. Once a client selects you, the quotation and AZPHUR project-payment allocation hub will appear here automatically.
        </p>
      </div>
    ) : (
      incomingLeads
        ...
        .map(lead => {
          ...
          const finalDownpayment = isProvider ? lead.provider_downpayment : lead.installer_downpayment;
          const finalBalance = isProvider ? lead.provider_balance : lead.installer_balance;
          const finalPercent = isProvider ? lead.provider_downpayment_pct : lead.installer_downpayment_pct;
          const finalQuoteLockKey = isProvider ? 'provider_final_quote_locked' : 'installer_final_quote_locked';
          const hasThisEntitySubmitted = Boolean(lead.quote_details?.[finalQuoteLockKey]);
          const savedFinalTotal = hasThisEntitySubmitted
            ? Number(finalDownpayment || 0) + Number(finalBalance || 0)
            : '';

          // Initial proposal values are only for customer selection.
          // The ACTIVE PROJECT final quote is independent and starts blank.
          const inputState = {
            basePrice: currentInput.basePrice !== undefined
              ? currentInput.basePrice
              : (isProvider && hasThisEntitySubmitted ? savedFinalTotal : ''),
            recommendedPrice: currentInput.recommendedPrice ?? '',
            downpaymentPercent: currentInput.downpaymentPercent ?? (hasThisEntitySubmitted ? Number(finalPercent) : 30),
            panelModel: currentInput.panelModel ?? '',
            inverterBattery: currentInput.inverterBattery ?? '',
            installPeriod: currentInput.installPeriod ?? '',
            workmanshipTerms: currentInput.workmanshipTerms ?? '',
            laborCost: currentInput.laborCost !== undefined
              ? currentInput.laborCost
              : (!isProvider && hasThisEntitySubmitted ? savedFinalTotal : '')
          };
          ...
          const baseNumeric = isProvider
            ? Number(inputState.basePrice || 0)
            : Number(inputState.laborCost || 0);
          const liveTaxBreakdown = calculateTaxBreakdown(baseNumeric);
          const dpPercent = Number(inputState.downpaymentPercent || 30);
          const calculatedDownpayment = (liveTaxBreakdown.total * dpPercent) / 100;
          const calculatedBalance = liveTaxBreakdown.total - calculatedDownpayment;
          const displayedDownpayment = hasThisEntitySubmitted ? Number(downpayment || 0) : calculatedDownpayment;
          const displayedBalance = hasThisEntitySubmitted ? Number(balance || 0) : calculatedBalance;
            ...
            <div key={lead.id} className="lead-card" style={{ background: '#fff', border: '2px solid #22d3ee', padding: '15px', borderRadius: '16px', marginBottom: '15px', boxSizing: 'border-box', position: 'relative', overflow: 'hidden', width: '100%' }}>
              ...
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '12px', marginBottom: '15px', boxSizing: 'border-box' }}>
                <div style={{ fontSize: '10px', fontWeight: 900, color: '#0284c7', letterSpacing: '0.5px', marginBottom: '10px', fontFamily: 'monospace', wordBreak: 'break-word' }}>
                  GENERATE_OFFICIAL_QUOTE ({taxProfile.vatRegistered ? `VAT ${taxProfile.vatRate}%` : 'NON-VAT'})
                </div>

                <div style={{ marginBottom: '12px' }}>
                  <input 
                    type="number" 
                    placeholder={isProvider ? 'Hardware Base Price (PHP)' : 'Installation Labor Cost (PHP)'}
                    value={isProvider ? inputState.basePrice : inputState.laborCost}
                    onChange={e => handleQuoteInputChange(lead.id, isProvider ? 'basePrice' : 'laborCost', e.target.value)}
                ...
                <button 
                  onClick={() => {
                    ...
                    saveDetailedQuotation(lead.id, entityType, true);
                  }}
                  disabled={isPriceLocked}
                  style={{ 
                    width: '100%', 
              ...
              <div style={{ background: '#f8fafc', border: '1px solid #cbd5e1', padding: '12px', borderRadius: '12px', marginBottom: '15px', boxSizing: 'border-box' }}>
                <div style={{ fontSize: '10px', fontWeight: 900, color: '#0284c7', letterSpacing: '0.5px', marginBottom: '10px', fontFamily: 'monospace', wordBreak: 'break-word' }}>
                  YOUR ALLOCATION IN THE AZPHUR PROJECT PAYMENT
                </div>
                
                <div style={{ fontSize: '10px', color: '#475569', fontFamily: 'monospace', marginBottom: '12px', wordBreak: 'break-word', lineHeight: '1.5' }}>
                  BASE: ₱{liveTaxBreakdown.base.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} | VAT: ₱{liveTaxBreakdown.vat.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}<br/>
                  YOUR DOWN-PAYMENT ALLOCATION: ₱{displayedDownpayment.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})} | YOUR FINAL ALLOCATION: ₱{displayedBalance.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', fontSize: '10px', color: '#475569', gap: '8px', marginBottom: '15px' }}>
                  <div style={{ width: '100%', background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', boxSizing: 'border-box' }}>
                    <span style={{ fontWeight: 700, color: '#334155' }}>Project Down Payment - Your Allocation:</span>
                  ...
                  <div style={{ width: '100%', background: '#fff', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', boxSizing: 'border-box' }}>
                    <span style={{ fontWeight: 700, color: '#334155' }}>Project Final Balance - Your Allocation:</span>
                ...
                {(hasThisEntitySubmitted || isPaidInitial) && (
                  <div style={{ marginTop: '15px' }}>
                    {!isManuallyUnlocked ? (
                      ...
                      >
                        APPROVE YOUR FINAL ALLOCATION FOR PROJECT PAYMENT
                      </button>
                    ) : (
                      <div style={{ background: '#e0f2fe', border: '1px solid #0284c7', padding: '10px', borderRadius: '8px', textAlign: 'center', boxSizing: 'border-box' }}>
                        <span style={{ fontSize: '10px', fontWeight: 900, color: '#0369a1', fontFamily: 'monospace', display: 'block', marginBottom: '6px', wordBreak: 'break-word' }}>
                          🔓 YOUR FINAL ALLOCATION APPROVED FOR CUSTOMER PROJECT PAYMENT
                        </span>
                        <button 
                ...
                {isPaidBalance && (
                  <div style={{ marginTop: '12px' }}>
                    ...
                    >
                      REMOVE COMPLETED RECORD FROM MY VIEW
                    </button>
                  </div>
                )}
              </div>
...
const renderThumbnails = (arr, label) => {
              ...
              {!isCurrentEntityAccepted ? (
                ...
                <div style={{ background: '#e0f2fe', border: '1px solid #0284c7', padding: '12px', borderRadius: '8px', boxSizing: 'border-box' }}>
                  ...
                  <div style={{ marginBottom: '10px', padding: '8px', borderRadius: '6px', background: '#fff', border: '1px solid #bae6fd', fontSize: '10px', fontWeight: 800, color: '#0369a1' }}>
                    TAX PROFILE: {taxProfile.vatRegistered ? `VAT REGISTERED (${taxProfile.vatRate}%)` : 'NON-VAT'} · PRICE MODE: {taxProfile.pricesIncludeTax ? 'TAX INCLUDED' : 'TAX EXCLUDED'}
                  </div>
                  ...
                  {entityType === 'provider' ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '11px' }}>
                      ...
                      <div>
                        <label style={{ fontWeight: 'bold', display: 'block', marginBottom: '2px' }}>Base Price (₱ {taxProfile.pricesIncludeTax ? 'Tax Included' : 'Tax Excluded'}):</label>
                        <input 
                          type="number" 
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '11px' }}>
                      ...
                      <div>
                        <label style={{ fontWeight: 'bold', display: 'block', marginBottom: '2px' }}>Labor Cost (₱ {taxProfile.pricesIncludeTax ? 'Tax Included' : 'Tax Excluded'}):</label>
                        <input 
                          type="number" 
